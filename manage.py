...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single'):
    ...
    if camera_type == "stereo":
        ...
        if cfg.DONKEY_GYM:
            print("Why am i using donkey gym ? ")
    ...
    if cfg.PUB_CAMERA_IMAGES:
        ...
        from donkeycar.parts.network import TCPServeValue, ZMQValuePub
        from donkeycar.parts.image import ImgArrToJpg
        pub = TCPServeValue("camera")
        p = ZMQValuePub("test")
        V.add(ImgArrToJpg(), inputs=['cam/image_array'], outputs=['jpg/bin'])
        V.add(pub, inputs=['jpg/bin'])
        V.add(p, inputs=['jpg/bin'])


    print("using joystick : %s" % use_joystick)        
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        ctr = cont_class(throttle_scale=cfg.JOYSTICK_MAX_THROTTLE * -1.0,
                                 steering_scale=cfg.JOYSTICK_STEERING_SCALE * -1.0,
    ...
    def led_cond(mode, recording, recording_alert, behavior_state, reloaded_model, track_loc):
        ...
        if track_loc is not None:
            led.set_rgb_values( *cfg.LOC_COLORS[track_loc])
            return -1

        if reloaded_model:
            led.set_rgb_values( cfg.MODEL_RELOADED_LED_R, cfg.MODEL_RELOADED_LED_G, cfg.MODEL_RELOADED_LED_B)
            return 0.1
        else:
            led.set_rgb_values( cfg.LED_R, cfg.LED_G, cfg.LED_B)

        if recording_alert:
            led.set_rgb_values( *recording_alert)
    ...
    if cfg.HAVE_IMU:
        inputs += ['imu/acc_x', 'imu/acc_y', 'imu/acc_z',
            'imu/gyro_x', 'imu/gyro_y', 'imu/gyro_z']
